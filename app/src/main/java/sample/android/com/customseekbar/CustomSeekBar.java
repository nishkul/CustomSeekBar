  ...
  /*  private void init(Context context, AttributeSet attrs) {
        ...
        if (attrs != null) {
            ...
            final TypedArray a = context.obtainStyledAttributes(attrs,
                    R.styleable.AppCompatImageView, 0, 0);
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        mProgressSweep = (float) mPoints / mMax * 360;
        Log.v("qqqqqqqq ww w", mProgressSweep + " ");
        mArcPaint = new Paint();
        mArcPaint.setColor(arcColor);
        mArcPaint.setAntiAlias(true);
        mArcPaint.setStyle(Paint.Style.STROKE);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        Log.v("qqqqqq  wwwwwww", mTranslateX + " " + mTranslateY);

        int arcDiameter = min - getPaddingLeft();
        Log.v("qqqqqq  wwwwwww", min + " " + getPaddingLeft());


        mArcRadius = arcDiameter / 2;
        Log.v("qqqqqq  wwwwwww", mArcRadius + " ");

        float top = height / 2 - (arcDiameter / 2);
        float left = width / 2 - (arcDiameter / 2);
        mArcRect.set(left, top, left + arcDiameter, top + arcDiameter);
        Log.v("qqqqqq  wwwwwww", top + " " + left);
...
//			System.out.printf("%d, %f\n", progress, mProgressSweep);
    ...
    public interface OnSwagPointsChangeListener {
         ...
         */
        void onPointsChanged(CustomSeekBar swagPoints, int points, boolean fromUser);

        void onStartTrackingTouch(CustomSeekBar swagPoints);

        void onStopTrackingTouch(CustomSeekBar swagPoints);
    }
}